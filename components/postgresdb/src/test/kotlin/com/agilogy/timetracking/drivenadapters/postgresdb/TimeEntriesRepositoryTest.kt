...
class TimeEntriesRepositoryTest : FunSpec() {
    ...
    private suspend fun <A> withPostgresTestRepo(initialState: TimeEntriesState, f: suspend (TimeEntriesRepository) -> A): Pair<A, TimeEntriesState> {
        ...
        return withTestDataSource { dataSource ->
            runMigrations(dataSource, clean = true)
            val testeeRepo = PostgresTimeEntriesRepository(dataSource)
            val testRepo = PostgresTimeEntriesTestRepository(dataSource)
            testRepo.insertAll(initialState.timeEntries)
            val result = f(testeeRepo)
            val finalEntries = testRepo.selectAll()
            val finalState = initialState.setTimeEntries(finalEntries)
            Pair(result, finalState)
        }
    }

    private suspend fun <A> withInMemoryTestRepo(initialState: TimeEntriesState, f: suspend (TimeEntriesRepository) -> A): Pair<A, TimeEntriesState> {
        val repo = InMemoryTimeEntriesRepository(initialState)
        return Pair(f(repo), repo.getState())
    }
...
//        val p2 = ProjectName("p2")
        ...
        fun <A> test(name: String, initialState: TimeEntriesState,
                     test: suspend (TimeEntriesRepository) -> A,
                     assertions: TestScope.(A, TimeEntriesState) -> Unit) {
            context(name) {
                this.test("Postgres") {
                    val (result, finalState) = withPostgresTestRepo(initialState, test)
                    assertions(result, finalState)
                }
                this.test("In Memory") {
                    val (result, finalState) = withInMemoryTestRepo(initialState, test)
                    assertions(result, finalState)
                }
            }
        }
        ...
        val getHoursInitialState = TimeEntriesState.empty().withTimeEntries(listOf(
                TimeEntry(d1, p, timePeriod(1, 9, 4), zoneId),
                TimeEntry(d1, p, timePeriod(1, 14, 3), zoneId),
                TimeEntry(d2, p, timePeriod(1, 10, 3), zoneId),
        ))
        val testDay = date(1)
        test("getHoursByDeveloperAndProject",
                getHoursInitialState,
                { it.getHoursByDeveloperAndProject(testDay.toLocalInstant()..testDay.plusDays(1).toLocalInstant()) }) { result, finalState ->
            assertEquals(
                    mapOf(
            ...
            assertEquals(getHoursInitialState, finalState)
        }
